...
namespace Foam
{
    defineTypeNameAndDebug(refinementHistory2D, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::refinementHistory2D::writeEntry
(
...
void Foam::refinementHistory2D::writeDebug
(
...
Foam::refinementHistory2D::splitCell4::splitCell4()
:
    parent_(-1),
    addedCellsPtr_(NULL)
{}
...
Foam::refinementHistory2D::splitCell4::splitCell4(const label parent)
:
    parent_(parent),
    addedCellsPtr_(NULL)
{}
...
Foam::refinementHistory2D::splitCell4::splitCell4(Istream& is)
{
    is >> *this;
}


Foam::refinementHistory2D::splitCell4::splitCell4(const splitCell4& sc)
:
    parent_(sc.parent_),
    addedCellsPtr_
    (
...
void Foam::refinementHistory2D::splitCell4::operator=(const splitCell4& s)
{
...
bool Foam::refinementHistory2D::splitCell4::operator==(const splitCell4& s) const
{
...
Foam::Istream& Foam::operator>>(Istream& is, refinementHistory2D::splitCell4& sc)
{
...
(
    Ostream& os,
    const refinementHistory2D::splitCell4& sc
)
{
...
void Foam::refinementHistory2D::checkIndices() const
{
    // Check indices.
...
Foam::label Foam::refinementHistory2D::allocateSplitCell
(
    const label parent,
    const label i
)
...
void Foam::refinementHistory2D::freeSplitCell(const label index)
{
...
void Foam::refinementHistory2D::markSplit
(
    const label index,
    labelList& oldToNew,
...
void Foam::refinementHistory2D::mark
(
    const label val,
    const label index,
...
Foam::label Foam::refinementHistory2D::markCommonCells
(
...
void Foam::refinementHistory2D::add
(
...
void Foam::refinementHistory2D::apply
(
...
Foam::refinementHistory2D::refinementHistory2D(const IOobject& io)
:
    regIOobject(io),
    active_(false)
{
    ...
    // that some processors do have refinementHistory2D and some don't so
    // test for active has to be outside of above condition.
    active_ = (returnReduce(visibleCells_.size(), sumOp<label>()) > 0);

    if (debug)
    {
        Pout<< "refinementHistory2D::refinementHistory2D :"
...
Foam::refinementHistory2D::refinementHistory2D
(
    const IOobject& io,
    const List<splitCell4>& splitCells,
    const labelList& visibleCells,
    ...
    visibleCells_(rh.visibleCells())
{
    if (debug)
    {
        Pout<< "refinementHistory2D::refinementHistory2D : constructed initial"
            << " history." << endl;
    }
}
...
(
    ...
    const UPtrList<const refinementHistory2D>& refs
)
:
    regIOobject(io),
    active_(false)
{
    ...
    )
    {
        WarningIn
        (
            "refinementHistory2D::refinementHistory2D(const IOobject&"
            ", const labelListList&, const PtrList<refinementHistory2D>&)"
...
Foam::refinementHistory2D::refinementHistory2D(const IOobject& io, Istream& is)
:
    regIOobject(io),
    splitCells_(is),
    freeSplitCells_(0),
...
Foam::autoPtr<Foam::refinementHistory2D> Foam::refinementHistory2D::clone
(
    const IOobject& io,
...
) const
{
    ...
    return autoPtr<refinementHistory2D>
    (
        new refinementHistory2D
        (
            io,
            newSplitCells,
            newVisibleCells,
...
void Foam::refinementHistory2D::resize(const label size)
{
    label oldSize = visibleCells_.size();

    if (debug)
    {
        Pout<< "refinementHistory2D::resize from " << oldSize << " to " << size
            << " cells" << endl;
    }
...
void Foam::refinementHistory2D::updateMesh(const mapPolyMesh& map)
{
    if (active())
    {
        ...
        if (debug)
        {
            Pout<< "refinementHistory2D::updateMesh : from "
                << visibleCells_.size()
...
void Foam::refinementHistory2D::subset
(
...
void Foam::refinementHistory2D::countProc
(
    const label index,
...
void Foam::refinementHistory2D::distribute(const mapDistributePolyMesh& map)
{
    if (!active())
    {
        FatalErrorInFunction
    ...
    //Pout<< "refinementHistory2D::distribute :"
...
void Foam::refinementHistory2D::compact()
{
    if (debug)
    {
        Pout<< "refinementHistory2D::compact() Entering with:"
    ...
    if (debug)
    {
        Pout<< "refinementHistory2D::compact : compacted splitCells from "
    ...
    if (debug)
    {
        Pout<< "refinementHistory2D::compact() NOW:"
            << " freeSplitCells_:" << freeSplitCells_.size()
            << " splitCells_:" << splitCells_.size()
...
void Foam::refinementHistory2D::writeDebug() const
{
    writeDebug(visibleCells_, splitCells_);
}


void Foam::refinementHistory2D::storeSplit
(
    const label celli,
...
void Foam::refinementHistory2D::combineCells
(
...
bool Foam::refinementHistory2D::read()
{
...
bool Foam::refinementHistory2D::readData(Istream& is)
{
    is >> *this;
    return !is.bad();
}
...
bool Foam::refinementHistory2D::writeData(Ostream& os) const
{
    os << *this;
...
Foam::Istream& Foam::operator>>(Istream& is, refinementHistory2D& rh)
{
...
Foam::Ostream& Foam::operator<<(Ostream& os, const refinementHistory2D& rh)
{
    const_cast<refinementHistory2D&>(rh).compact();
